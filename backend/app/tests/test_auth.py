...
def test_get_current_user_raises_401_when_token_cannot_be_decoded() -> None:
    with patch("app.core.dependencies.decode_access_token") as mock_decode_access_token:
        ...
        with pytest.raises(HTTPException) as exc_info:
            get_current_user(token="bad-token", db=Mock())

        assert exc_info.value.status_code == 401
        mock_decode_access_token.assert_called_once_with("bad-token")


def test_get_current_user_raises_401_when_sub_claim_is_missing() -> None:
    with patch("app.core.dependencies.decode_access_token") as mock_decode_access_token:
        mock_decode_access_token.return_value = {}

        with pytest.raises(HTTPException) as exc_info:
            get_current_user(token="token-without-sub", db=Mock())

        assert exc_info.value.status_code == 401
        mock_decode_access_token.assert_called_once_with("token-without-sub")


def test_get_current_user_raises_401_when_sub_claim_is_not_an_int() -> None:
    with patch("app.core.dependencies.decode_access_token") as mock_decode_access_token:
        mock_decode_access_token.return_value = {"sub": "not-an-int"}

        with pytest.raises(HTTPException) as exc_info:
            get_current_user(token="token-with-invalid-sub", db=Mock())

        assert exc_info.value.status_code == 401
        mock_decode_access_token.assert_called_once_with("token-with-invalid-sub")


def test_get_current_user_raises_401_when_user_does_not_exist() -> None:
    with (
        ...
        with pytest.raises(HTTPException) as exc_info:
            get_current_user(token="valid-token", db=db)
...
def test_get_current_user_returns_user_for_valid_token() -> None:
    ...
    with (
        ...
        current_user = get_current_user(token="valid-token", db=db)
